// --- Core Data: Game and Site Configuration ---
const gamesData = [
...
// --- Core Data: Widget Configuration ---
...
// --- 2. THEME AND BACKGROUND SETTINGS ---
...
function renderDashboardWidgets() {
    ...
    activeWidgets.forEach(widgetId => {
        ...
        const storedSize = localStorage.getItem(`widgetSize-${widgetId}`) || config.initialSize; 
    ...
    // Re-attach proxy search listener to new quick search input
...
// --- 6. ATLAS PROXY LOGIC (UPDATED FOR IFRAME) ---
...
function showSuggestions(inputValue) {
    ...
    filteredSites.slice(0, 5).forEach(site => { 
        ...
        suggestionItem.onclick = () => {
            ...
            suggestionsList.innerHTML = ''; 
            // Automatically launch proxy when suggestion is clicked
...
function handleProxySearch(quickUrl = null) {
    ...
    // If quickUrl is not provided, check the main input
    if (!url && searchInput) {
        url = searchInput.value.trim();
        // Clear main input after search
        searchInput.value = '';
        const suggestionsList = document.getElementById('proxy-suggestions');
        if (suggestionsList) suggestionsList.innerHTML = '';
    }
    
    // Also check the quick search widget input if we're on the dashboard
    if (!url) {
    ...
    // === MOCKUP: Inject Iframe into the Atlas Proxy Section ===
    const container = document.getElementById('proxy-content-container');
    if (!container) return;

    // Use a placeholder URL for the iframe src if it will not work due to network security.
    // However, since the user wants to *see* the site, we use the raw URL.
    // NOTE: Many sites (like Google, YouTube) prevent loading in an iframe due to X-Frame-Options.
    // The real proxy script running on the server would handle this by rewriting the content.
    ...
    // If the search was initiated from the quick widget, navigate to the proxy page
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Load Settings (Theme, Background, Opacity)
    applySavedSettings(); 
    applySavedOpacity(); 
    ...
    window.addEventListener('hashchange', () => {
        navigateToPage(window.location.hash);
        // Ensure proxy view is reset if we navigate away and return
        if (window.location.hash === '#atlas-proxy') {
            closeProxyView(); 
        }
    });
    ...
    // 4. Initialize Settings & Widgets UI
    handleOpacitySlider(); 
    ...
    // Event Listeners for Atlas Arcade
    loadGames(gamesData); 
    ...
    // Event Listeners for Atlas Proxy (Main Search Bar)
    attachProxyEventListeners();
    
    // Event Listener for Atlas AI
